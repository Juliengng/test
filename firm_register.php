        ...
        <div class="row">
            <div class="col-md-6 col-md-offset-3">
                <form class="form-signin mg-btm" method="post" action="">
                    <h1 class="heading-desc" style="background-color: #504e63">Create an account</h1>
                    <div class="main">
                        ...
                        <input type="email" name="confirmemail" id="confirmemail" placeholder="Email confirmation" value="" required="" class="form-control" autofocus>
                        <input type="checkbox" name="cgu" value="cgu" required ><span style="color:black"> I accept the CGU</span>
                    </div>
                    <div class="login-footer">
                        <div class="row">
                            <div class="col-xs-6 col-md-6 pull-right">
                                <button type="submit" class="btn btn-large btn-primary pull-right" value="submit" name="next" style="border: solid darkorange 2px;background-color: #504e63;">Create company account</button>
                            </div>
                        </div>
                    </div>
                    <?php
                    ...
                    if(isset($_POST['next'])){
                        ...
                        if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){
                            ...
                            $sql = "SELECT `FirmEmail` FROM `firm` WHERE FirmEmail = '$email'";
                            $result = mysqli_query($link, $sql);
                            $row = mysqli_fetch_row($result);
                    ...
                    if(empty($error[0])){
                        ...
                        if (isset($_POST['next'])&& $_POST['next']== "submit"){
                            ...
                            $sql = "INSERT INTO `user`(`usrName`, `usrType`) VALUES ('$id', 'firm');";
                            mysqli_query($link, $sql);
                            
                             $sql = "INSERT INTO `firm`(`FirmUsrName`, `FirmPassword`, `FirmEmail`,  `FirmApproved`) VALUES ('$id','$password','$email',1);";

                            if(mysqli_query($link, $sql)){

                                $_SESSION['username'] = $id;
                                $_SESSION['role'] = 'firm';
                                echo '<meta http-equiv=REFRESH CONTENT=0;url=FirmInfo.php>';
