  ...
  <body>
          ...
          <div class="row">
            <div class="col-lg-12">
              ...
              <div class="form-group">
                <div class="input-group col-sm-8">
                  <div class="input-group-addon"><span class="glyphicon glyphicon-usd"></span></div>
                  <input type="text" class="form-control" name="Living_cost" placeholder="Living_cost" value="<?php if(isset($_POST['Living_cost'])){ echo $_POST['Living_cost']; } ?>">
                </div>
              </div>
    ...
    <?php
      ...
      if(empty($_POST['JobTitle'])){
        $error[] = 'Please fill in the job title for the internship.';
      }
      if(empty($_POST['JobCategory'])){
        $error[] = 'Please fill in the the job category.';
      }
      if(empty($_POST['Description'])){
        $error[] = 'Please discript the mainly task for the internship.';
      }
      if(empty($_POST['starting_date'])){
        $error[] = 'Please select the starting date of the internship.';
      }
      if(empty($_POST['ending_date'])){
        $error[] = 'Please select the ending date of the internship.';
      }
    ...
    <?php
      ...
      if(empty($error[0])) {
          ...
          if (isset($_POST['submit'])&& $_POST['submit']== "submit"){
            $insert = "INSERT INTO `internship`(`FirmUsrName`, `JobCategory`, `Quota`, `JobTitle`, `Salary`, `Language`, `Description`, `Location`, `PostedBy`, `InternAgreement`, `DurationStart`, `DurationEnd`, `LivingCost`)
                       VALUES ('$id','$JobCategory',$Quota,'$JobTitle',$Salary,'$Language','$Description','$Country','$id','','$starting','$ending', $LivingCost)";

            $result = mysqli_query($link, $insert) or trigger_error("Query Failed! SQL: $insert - Error: ".mysqli_error(), E_USER_ERROR);;
            $internshipID = mysqli_insert_id($link);

			  foreach ($_POST['Skills'] as $s) {
				  $insertSkill= "INSERT INTO `InternshipSkills`(`InternshipID`,`SkillID`) VALUES ('$internshipID','$s')";
        ...
        foreach ($_POST['PersonalSkills'] as $ps) {
				  $insertPersonalSkill= "INSERT INTO `InternshipPersonalSkills`(`InternshipID`,`PersonalSkillID`) VALUES ('$internshipID','$ps')";
